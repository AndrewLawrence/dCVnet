...
plot.repeated.cv.glmnet <- function(obj) {
  obj$lambda <- log10(obj$lambda)
  #average <- average_repeated.cv.glmnet(obj)
  p <- ggplot2::ggplot(obj,
                       ggplot2::aes(y = cvm, x = lambda)) +
    ggplot2::geom_line(data = obj, colour = "grey", size = 1.2) +
    ggplot2::geom_point(data = obj[obj$lambda.min,],
                        ggplot2::aes(y = cvm, x = lambda),
                        shape = 25, size = 2, colour = "red", fill = "black",
                        inherit.aes = F) +
    ggplot2::ylab(paste("Metric:", attr(obj, "type.measure"))) +
...
plot.multialpha.repeated.cv.glmnet <- function(obj) {
  obj <- obj$inner_results # only need the inner_results dataframe.

  p <- ggplot2::ggplot(obj,
                       ggplot2::aes(y = cvm, x = log10(lambda),
                                    colour = alpha, fill = alpha)) +
    ggplot2::geom_line() +
    ggplot2::geom_point(data = obj[obj$lambda.min,],
                        colour = "black", shape = 25) +
    ggplot2::geom_point(data = obj[obj$best,],
...
plot.dCVnet <- function(object, type = "tuning", ...) {
  # options:
  #   "tuning"
  #   "ROC"
  in_type <- type
  ...
  type <- type_opts[type]
  switch(type,
         tuning = return(tuning_plot_dCVnet(object, ...)),
         roc = return(plot(rocdata(classperformance(object)), ...))
  )
}


plot.rocdata <- function(rocdata, legend = F) {

  p <- ggplot2::ggplot(rocdata, ggplot2::aes(y = Sens,
                                             x = InvSpec,
                                             group = run,
                                             colour = run)) +
...
plot_outerloop_coefs <- function(object) {
  ...
  p <-  ggplot2::ggplot(df,
                        ggplot2::aes(y = StdBeta, x = Predictor,
                                     colour = StdBeta)) +
