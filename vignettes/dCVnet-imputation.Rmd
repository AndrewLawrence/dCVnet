...
Above we have simulated Missing At Random (MAR) missingness in a single variable. The values selected for removal were ten of the larger values of the variable `lcp`. Specifically, there is a strong correlation between `lcp` and `svi`, and the missing `lcp` values were selected at random among subjects with high `svi`. Thus the missing values are MAR because they are typical `lcp` values given the observed values of `svi`. They are not MCAR because they are not unconditionally typical of `lcp` values. They are not MNAR because the probability of missingness depends only on observed values (of `svi`).
...
Imputation in dCVnet is controlled by two option arguments:

-   `opt.use_imputation` (`FALSE`/`TRUE`) - determines if imputation is used.

-   `opt.imputation_method`, specifies the imputation methods (if being used):
...
When `opt.use_imputation` is set to `TRUE` dCVnet will use the imputation method specified by `opt.imputation_type` to impute missing values in the predictor matrix.

## Example

To demonstrate imputation in dCVnet we will fit (and cross-validate) five dCVnet models to the prostate dataset above:
...
-   `"knnimp"` and `"rfimp"` - These are conditional imputation methods so should produce better imputations of missing `lcp` values than `"meanimp"`. Hopefully this will recover the performance difference between `"orig"` and `"cc"`.

Note, below I fit the models in a loop using a settings object to determine which particular model is fit. This is to avoid writing out the call to dCVnet five times. In typical usage one should just add any imputation arguments to a standard dCVnet call: `dCVnet(my_outcome, data = my_data, ..., opt.use_imputation = TRUE, opt.imputation_method = "knn")`.
